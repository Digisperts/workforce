...
export default function Navbar() {
  ...
  return (
    <nav
      className={`fixed top-4 left-1/2 -translate-x-1/2 w-[92%] md:w-[90%] backdrop-blur-lg border rounded-2xl shadow-lg z-50 transition-all duration-300 ${scrolled ? "bg-white border-gray-200" : "bg-white/10 border-white/20"
        }`}
    >
      <div className="flex items-center justify-between px-6 md:px-10 py-3">
        ...
        {/* Desktop Menu */}
        <div
          className={`hidden md:flex items-center space-x-8 font-small transition-colors duration-300 ${scrolled ? "text-black" : "text-white"
            }`}
        >
          {navItems.map((item) => (
            <Link
          ...
          >
            <button
              ...
              className={`flex items-center gap-1 cursor-pointer transition-colors duration-300 ${pathname.startsWith("/resources")
                  ? "text-[#FF7F50]"
                  : scrolled
                    ? "hover:text-[#FF7F50] text-black"
                    : "hover:text-[#FF7F50] text-white"
            ...
            {dropdownOpen && (
              <div
                className={`absolute left-0 top-full w-44 rounded-md py-2 transition-all duration-200 z-100 ${scrolled
              ...
              >
                {["Blog", "Guides", "Contact"].map((item, idx) => (
                  <Link
                    key={idx}
                    href="#"
                    className={`block px-4 py-2 text-sm transition ${scrolled
        ...
        <div className="hidden md:flex items-center space-x-4 transition-colors duration-300">
          <Link
            ...
            className={`px-5 py-2 rounded-md font-medium transition ${scrolled
          ...
          <Link
            href="/auth/candidate/get-started"
        ...
        <button
          ...
          className={`md:hidden text-2xl transition-colors duration-300 ${scrolled ? "text-black" : "text-white"
            }`}
      ...
      {isOpen && (
        <div
          className={`md:hidden transition-all duration-300 rounded-b-2xl ${scrolled
        ...
        >
          <div className="flex flex-col items-center space-y-4 py-6 font-medium">
            ...
            <div className="flex flex-col space-y-3 pt-4">
              <Link
                href="/auth/candidate/login"
                className={`px-6 py-2 rounded-md font-medium transition ${scrolled
                    ? "border border-black text-black hover:bg-gray-100"
                    : "border border-white text-white hover:bg-white/10"
                  }`}
              >
